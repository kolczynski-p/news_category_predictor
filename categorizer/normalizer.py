...
class Normalizer():
    ...
    def to_lower(self, cols):
        for c in cols:
            self.data[c] = self.data[c].map(
                lambda x: x if type(x) != str else x.lower())

    def drop_num(self, cols):
        for c in cols:
            self.data[c] = self.data[c].apply(lambda x: " ".join(
                x for x in x.split() if not x.isdigit()))
    ...
    def drop_rarest(self, amount):
        ...
        self.data['text'] = self.data['text'].apply(
            lambda x: " ".join(x for x in x.split() if x in freq))
